...
// Let's turn it into a faucet-pipeline pipeline!
...
function run(watcher) {
	return process => {	// We turn the configuration into a function process
...
function build(config, assetManager, options) {
	...
	return files => {	// Now we build the processing function, which again takes an optional Array of files to process
		...
		// If our pipeline reads or depends on other files, we’ll want to keep going as well
		// For example, when we’re bundling a script entry point, we should run when its dependencies change
...
function apply(buffer, { skip }, { compact }) {
	...
	let contents = buffer.toString();	// We read a Buffer, but rot13 works on Strings.
	// We could have passed {encoding: 'utf8'} to readFile above instead of converting the buffer here.
